...
%DIF DEL gershwin34-old.tex   Thu Jun 12 00:01:26 2014
%DIF ADD gershwin34-new.tex   Thu Jun 12 00:01:26 2014
%DIF < \documentclass[jgr]{agu2001}
%DIF -------
\documentclass[jgrga]{agu2001} %DIF > 
%DIF -------
%\documentclass[draft,jgr]{agu2001}
%DIF 3-4c3-7
%DIF < %\usepackage{times}
%DIF < \bibliographystyle{agu}
%DIF -------
%\usepackage{pslatex} %DIF > 
% EXPERIMENTAL for LANDSCAPE FIGURE %DIF > 
%\usepackage{lscape} %DIF > 
 %DIF > 
\bibliographystyle{agu04} %DIF > 
%DIF -------
\usepackage{mylatex}
% NO INCLUDE LABEL
\renewcommand{\includefig}[2]{}
%DIF 8c11
%DIF < % GRAPHICS uncomment the next two lines to include graphics
%DIF -------
%% IF GRAPHICS uncomment the next two lines to include graphics %DIF > 
%DIF -------
\usepackage{graphicx}
...
%DIF -------
%% ELSE GRAPHICS %DIF > 
%\providecommand{\includegraphics}[2][dummy]{} %DIF > 
%\renewcommand{\includegraphics}[2][dummy]{} %DIF > 
%% ENDIF GRAPHICS %DIF > 
% simple ignore graphics include if there is no graphics %DIF > 
% Change markup %DIF > 
\RequirePackage[normalem]{ulem} %DIF > 
\RequirePackage{color} %DIF > 
\renewcommand{\chg}[1]{{\textcolor{blue}{\sf #1}}} %DIF > 
\renewcommand{\del}[1]{{\textcolor{red}{\sout{#1}}}} %DIF > 
%DIF -------
% PUBLISH: uncomment to make remark, chg and rem tags invisible
%DIF 13-15c25-27
%DIF < \renewcommand{\chg}[1]{#1}
%DIF < \renewcommand{\del}[1]{}
%DIF < \renewcommand{\remark}[1]{}
%DIF -------
%\renewcommand{\chg}[1]{#1} %DIF > 
%\renewcommand{\del}[1]{} %DIF > 
%\renewcommand{\remark}[1]{} %DIF > 
%DIF -------
...
%DIF 35c47
%DIF < GEOMAR, Wischofstr. 1-3, 24148 Kiel, Germany
%DIF -------
IFM-GEOMAR, Wischofstr. 1-3, 24148 Kiel, Germany %DIF > 
%DIF -------
...
\title{\DIFdelbegin \DIFdel{Micro-earthquake }\DIFdelend \DIFaddbegin \DIFadd{Microearthquake }\DIFaddend seismicity of the Mid-Atlantic ridge at
...
\DIFdelbegin %DIFDELCMD < \affil{GEOMAR, Kiel}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affil{IFM-GEOMAR, Kiel}
\DIFaddend \date{\today}

\begin{abstract}
\DIFdelbegin \DIFdel{Based on }\DIFdelend \DIFaddbegin 

\DIFadd{We report measurements made with }\DIFaddend an ocean-bottom \DIFdelbegin \DIFdel{micro-earthquake survey of }\DIFdelend \DIFaddbegin \DIFadd{array which was
operated for 10 days on }\DIFaddend the
Mid-Atlantic ridge just south of the 5\dg S transform fault/fracture
zone\DIFdelbegin \DIFdel{, we find seismic activity }\DIFdelend \DIFaddbegin \DIFadd{.  A total of 148 locatable earthquakes with magnitudes $\sim$0.5--2.8 were recorded;
seismic activity appears }\DIFaddend to be concentrated within the western
...
seismic activity to \DIFdelbegin \DIFdel{arise from slip on two parallel low-angle normal faults which dip from the inside corner toward the spreading
axis}\DIFdelend \DIFaddbegin \DIFadd{primarily arise from normal faulting}\DIFaddend . 
...
\citep[see e.g.,][]{neumann93}}%DIFAUXCMD
}\DIFaddend .
...
Here we report on the \DIFdelbegin \DIFdel{micro-earthquake }\DIFdelend \DIFaddbegin \DIFadd{microearthquake }\DIFaddend activity of the Mid-Atlantic
...
(\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Plate}\DIFaddend ~\ref{fig:seismag-map}).  This peculiarity and a number of other
...
\citet{tucholke94} model: the seismicity suggests \DIFdelbegin \DIFdel{that at least two parallel low-angle normal
faults are active at the same time
rather than extension being taken up by one fault only.
  These
}\DIFdelend \DIFaddbegin \DIFadd{normal
faulting is active beneath the western half of the median valley.
  The
currently active }\DIFaddend faults are 
 not related to the formation of the corrugations on the
domal massif but rather \DIFdelbegin \DIFdel{represent }\DIFdelend \DIFaddbegin \DIFadd{represents }\DIFaddend a more recent phase in the evolution
of the segment.
...
\citep{planert-con03}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citetext{see \citealp{planert-con03}, and section~\ref{sec:refraction} below}}%DIFAUXCMD
}\DIFaddend .
\end{enumerate}
...
We \DIFdelbegin \DIFdel{re-located }\DIFdelend \DIFaddbegin \DIFadd{relocated }\DIFaddend all events in the restricted event set at the same time
as determining \DIFdelbegin \DIFdel{a minimum
}\DIFdelend \DIFaddbegin \DIFadd{station corrections and the minimum
}\DIFaddend 1D-model\DIFdelbegin \DIFdel{and station corrections }\DIFdelend \DIFaddbegin \DIFadd{, i.e., the model which achieves the best fit of the traveltimes }\DIFaddend \citep[using VELEST,][]{kissling94}.  Since the event set is rather small, and most events
fall into the 7--11 km depth band \DIFaddbegin \DIFadd{(below sea level)}\DIFaddend , the 
...
%DIF <  to sea-level; in order to get depth below the median valley seafloor
%DIF >  to sea level; in order to get depth below the median valley seafloor
...
%8~km/s. 
  \DIFdelbegin \DIFdel{Otherwise, the model is strongly influenced by the starting 
model.  }\DIFdelend We considered two extreme  \DIFdelbegin \DIFdel{starting }\DIFdelend models, one appropriate for
...
inferred location between the different velocity models \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend small, such
that the patterns reported in the following are not affected by the
choice of velocity model (see \DIFdelbegin \DIFdel{appendix}\DIFdelend \DIFaddbegin \DIFadd{Figures~\ref{fig:mapvar}--\ref{fig:croslatvar} in supplementary
online material}\footnote{\DIFadd{Supporting material is available via Web browser or via Anonymous FTP
...
mislocations (\DIFdelbegin \DIFdel{see appendix for details}\DIFdelend \DIFaddbegin \DIFadd{Figures~\ref{fig:map95lat} and~\ref{fig:map95lon}
in online supplement}\DIFaddend ).  
Subsequently, we \DIFdelbegin \DIFdel{re-located }\DIFdelend \DIFaddbegin \DIFadd{relocated }\DIFaddend all marginal events with the station
terms and, where applicable, the velocity model derived from the
restricted data set. 

We further \DIFdelbegin \DIFdel{re-located }\DIFdelend \DIFaddbegin \DIFadd{relocated }\DIFaddend the events using the
double-difference method \citep{waldhauser00}\DIFaddbegin \DIFadd{, trying both an inversion using manual
picks only and an inversion using a  combination of manual and cross-correlation picks}\DIFaddend .  Whereas the results
...
(\DIFdelbegin \DIFdel{red }\DIFdelend lines in Figure~\ref{fig:glob-seis}; of the short profiles, only
the median valley profile is shown for clarity).  \DIFdelbegin \DIFdel{The long
...
concentrates on the segment north of the 5}%DIFDELCMD < \dg %%%
\DIFdel{S transform fault.
}\DIFdelend %extend from the center of one segment across the transform well into
%the next segment.
\DIFdelbegin \DIFdel{The short profiles  run nearly in north-south
...
}\DIFdelend % The four long profiles focused on a number of topographic features
...
% slow spreading ridge transform intersections. 
 \DIFaddbegin \DIFadd{Only a brief overview  is given here; the data
and analysis were described by \mbox{%DIFAUXCMD
\citet{planert-con03} }%DIFAUXCMD
and will be
presented in more detail elsewhere.
}\DIFaddend 

Because \DIFaddbegin \DIFadd{of }\DIFaddend the rough topography  \DIFdelbegin \DIFdel{caused enhanced attenuation of the seismic
energy propagation efficiency varies }\DIFdelend \DIFaddbegin \DIFadd{the propagation efficiency varied }\DIFaddend strongly but in most cases
...
the data.  \DIFdelbegin \DIFdel{For assessing velocity models we chose }\DIFdelend \DIFaddbegin \DIFadd{Velocity models were determined by }\DIFaddend a combination of
...
%\citep[e.g.]{white92}.
\DIFdelbegin \DIFdel{In the median valley the models show an unusually thin crust
of about 4~km
thickness (near obh04 in the middle of the profile, see Figure~\ref{fig:croslon})
underlain by a normal-to-low
velocity upper mantle ($V_p\sim$7.5~}\DIFdelend \DIFaddbegin 

\DIFadd{The median valley model is based on a re-analysis of the data
shown in \mbox{%DIFAUXCMD
\citet[Figures 5 and 6]{reston02}}%DIFAUXCMD
.  The Moho is found 3--5~km
...
uniform between the seafloor ($V_P\sim3$}\DIFaddend km/s) \DIFdelbegin \DIFdel{.  The Moho is shallowing
...
gradient , starting with sub-seafloor velocities of about 3~km}\DIFdelend \DIFaddbegin \DIFadd{and 2--3~km depth
($V_P\sim 6$km}\DIFaddend /\DIFdelbegin \DIFdel{s
increasing to lower crustal velocities of max. }\DIFdelend \DIFaddbegin \DIFadd{s). The lower-crustal gradient is then more gentle,
with velocities increasing to up to }\DIFaddend 6.9~km \DIFaddbegin \DIFadd{at the Moho, which is
underlain by normal-to-low-velocity upper mantle ($V_P\sim$7.5km}\DIFaddend /s\DIFaddbegin \DIFadd{)}\DIFaddend .
 In contrast\DIFdelbegin \DIFdel{velocity depth profiles in the region of the ICH as
well as }\DIFdelend \DIFaddbegin \DIFadd{,
 the ICH and
 }\DIFaddend the outside corner \DIFdelbegin \DIFdel{show either very }\DIFdelend \DIFaddbegin \DIFadd{have }\DIFaddend high near surface
velocities (\DIFaddbegin \DIFadd{e.g., }\DIFaddend $>$6.0km/s near obs06, see Figure~\ref{fig:croslat}) or very high velocity
gradients\DIFdelbegin \DIFdel{resulting in  }\DIFdelend \DIFaddbegin \DIFadd{, such that }\DIFaddend crustal velocities up to 6.5~km/s \DIFaddbegin \DIFadd{are attained }\DIFaddend within the
...
mantle, the models suggest \DIFdelbegin \DIFdel{crustal thicknesses }\DIFdelend \DIFaddbegin \DIFadd{a crustal thickness }\DIFaddend of 4.0-5.0~km at the
eastern flank of the inside corner high and 4.5-5.5~km at the outside
corner (Figure~\ref{fig:croslat}).
\DIFdelbegin \DIFdel{Further refinements including amplitude modeling is still in
progress and will be presented elsewhere.
}\DIFdelend 

\section{Discussion}

\subsection{\DIFdelbegin \DIFdel{How representative are the results of long term
  seismicity and comparison }\DIFdelend \DIFaddbegin \DIFadd{Comparison }\DIFaddend with teleseismically recorded earthquakes}

\DIFdelbegin \DIFdel{The interpretation of  seismicity patterns derived from brief deployments requires
the  assumption that these patterns are indicative
of long term trends.
%DIF < , even though swarm-like activity is known to
%DIF < occur frequently for large earthquakes (magnitude 5--6) at mid-oceanic
%DIF < ridges \citep{sykes70}.   
...
span a full seismic cycle, there is no way to answer this question with
 certainty}\DIFdelend \DIFaddbegin \DIFadd{A brief survey as presented here can offer nothing but a snapshot
of seismic activity}\DIFaddend . Nevertheless, we \DIFdelbegin \DIFdel{will put forward a number of
...
}\DIFdelend \DIFaddbegin \DIFadd{note 
%DIF > , even though swarm-like activity is known to
%DIF > occur frequently for large earthquakes (magnitude 5--6) at mid-oceanic
%DIF > ridges \citep{sykes70}.   
}

\DIFadd{that the recorded microearthquakes are unlikely to be
}\DIFaddend aftershocks of a large earthquake just before the deployment of the
ocean bottom network\DIFdelbegin \DIFdel{.  We estimate that an earthquake rupturing the whole
MVSZ would have a magnitude of
6.5.  Even an event rupturing along a length of 10 km would still have a magnitude of 4.5--5.  Earthquakes of such magnitude are large enough
to be recorded
globally,
but no event near the study area is listed in
the NEIC catalog \mbox{%DIFAUXCMD
\citep{neic}
}%DIFAUXCMD
either immediately preceding the experiment, or shortly afterwards.
The }\DIFdelend \DIFaddbegin \DIFadd{, as such an event would have been recorded
teleseismically.
 Also,
the }\DIFaddend fact that magnitudes estimated from the corner frequency agree at
...
data set for this part of the MAR \DIFdelbegin \DIFdel{also }\DIFdelend argues against the possibility that
we caught an aftershock or swarm sequence with
seismicity rates strongly exceeding long term averages.  
\DIFdelbegin \DIFdel{As outlined
...
\mbox{%DIFAUXCMD
\citep{toomey88,kong92,wolfe95,barclay01}}%DIFAUXCMD
, lasting between 10 and 43
...
in the North Atlantic \mbox{%DIFAUXCMD
\citep{smith03}}%DIFAUXCMD
, which has been operating 
since 2000. Both the spatial pattern
\mbox{%DIFAUXCMD
\citep[Figure~13 of ][]{smith03} }%DIFAUXCMD
and rate of seismic
...
faulting. The dip is $\sim 45\dg$ for both nodal planes\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{. Although
this is
}\DIFaddend 15--20{\dg} steeper than the low-angle plane inferred from the local
composite solution\DIFdelbegin \DIFdel{. However, the centroid depth has been fixed at  15~km,
which is very likely too deep given the results of this and other OBS
surveys and  the maximum centroid depth limit
of 6--7~km for slow spreading ridges that \mbox{%DIFAUXCMD
\citet{huang88} }%DIFAUXCMD
inferred  from waveform modeling. 
The dip trades off with depth and is poorly constrained by the CMT method if the depth is incorrect
\mbox{%DIFAUXCMD
\citep{dziewonski81a}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, this difference is probably not significant,
given the uncertainties inherent in both the local and CMT solutions.
}\DIFaddend %\remark{STATEMENT REALLY PERS. COMM. FROM
...
solutionsis probably not significant.
}\DIFdelend \DIFaddbegin 

 \DIFaddend Similarly to the results of the
...
\DIFdelbegin \DIFdel{Turning our attention now to the
}\DIFdelend \DIFaddbegin \DIFadd{We now consider the
central }\DIFaddend ridge-perpendicular \DIFdelbegin \DIFdel{cross-sections
}\DIFdelend \DIFaddbegin \DIFadd{cross-section
}\DIFaddend (Figure~\ref{fig:croslat} \DIFdelbegin \DIFdel{) we  attempt to constrain the faulting
styles based on the depth distribution of events and the focal solution.    We begin the discussion with the middle section, which includes the
largest number of events.  }\DIFdelend \DIFaddbegin \DIFadd{middle).    }\DIFaddend The composite focal solution for this area
...
\DIFdelbegin \DIFdel{High angle
faulting, if extending towards the surface , would imply a fault trace
on the outside corner side.  Indeed a fault,
labelled }\DIFdelend \DIFaddbegin \DIFadd{The surface traces of two faults bounding the MV (F1 on the inside corner side,
}\DIFaddend F2 \DIFdelbegin \DIFdel{in Figure}\DIFdelend \DIFaddbegin \DIFadd{on the outside corner, Plate}\DIFaddend ~\ref{fig:seismag-map},
\DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{fig:croslat}) were }\DIFaddend identified previously 
from the bathymetric data\DIFdelbegin \DIFdel{on the outside corner side of the median valley
\mbox{%DIFAUXCMD
\citep{reston02} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend and, although subdued at the latitude of the
cross-section, \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend still identifiable as a step in the bathymetric
profile. \DIFdelbegin \DIFdel{However, a number of events are just 1--2~km below the western part of the median valley. This seems hard to reconcile with a
single high angle fault originating at the eastern side wall. In
fact, several closely spaced high angle faults
...
valley \mbox{%DIFAUXCMD
\citep{reston02} }%DIFAUXCMD
and, like F2, dies out towards the north.  A
...
plane, inconsistent with the
focal solution.  But again a line can be
...
rather than by exhumation along a detachment fault\mbox{%DIFAUXCMD
\citep{reston02}}%DIFAUXCMD
.   Nevertheless it seems plausible that a fault
...
parallel fault planes dipping towards the median valley at $\sim$}\DIFdelend \DIFaddbegin \DIFadd{Whereas the event distribution is
clearly incompatible with the pattern expected for a single dominant
detachment fault \mbox{%DIFAUXCMD
\citep[e.g.][]{tucholke94}}%DIFAUXCMD
...
}\DIFaddend 30\dg. \DIFdelbegin \DIFdel{Four intermediate depth events
lie between the shallow and deep groups and are also aligned along a
30}%DIFDELCMD < {\dg} %%%
\DIFdel{dipping plane.  These events could be mislocated events on one
of the two more active faults, in
which case the apparent linear trend would have arisen
co-incidentally, or
another minor fault exists between them, running parallel to them.
}\DIFdelend \DIFaddbegin \DIFadd{The projection of the inner (eastern) fault would emerge near the
...
\DIFaddend %Whilst other tectonic systems are conceivable the one described is the
%simplest one compatible with both the focal mechanism and the
%distribution of events.

\DIFdelbegin \DIFdel{Somewhat puzzling about our
...
%DIF < and somewhat speculative 
possibility is that the fault is effectively
smeared by serpentinite fault gouge, lowering its seismic potential \mbox{%DIFAUXCMD
\citep{reinen00}}%DIFAUXCMD
;
serpentinite gouge was conjectured to have existed along a detachment
fault at an earlier phase of the segment evolution \mbox{%DIFAUXCMD
\citep{reston02}}%DIFAUXCMD
. This interpretation would imply that water
has penetrated into the mantle along this fault and caused localised
serpentinization but
}\DIFdelend %DIF > \del{fault gouge} \chg{
%DIF > }



\DIFdelbegin \DIFdel{serpentinization cannot be wide-spread as argued above from
...
%DIFDELCMD < %%%
\DIFdelend Moving on to the southernmost cross-section (Figure \ref{fig:croslat}, bottom)
we recognize a cluster of events at 9--10~km depth \DIFaddbegin \DIFadd{below
sea level }\DIFaddend and two isolated
events, one deep, one shallow further east.  \DIFdelbegin \DIFdel{Since the events in the
cluster are very closely spaced no linear trend can be
determined. Planes through the group of events at the angles }\DIFdelend \DIFaddbegin \DIFadd{Given the short
...
slopes of the faulting being }\DIFaddend suggested by the focal \DIFdelbegin \DIFdel{solution emerge either near fault trace F2 or
near the western scarp of the southern extension of the ICH ridge.  Both
...
a change in tectonic regime}\DIFdelend \DIFaddbegin \DIFadd{mechanism) or
with the recording of a fortuitous subset of events associated with more distributed faulting}\DIFaddend .
  Either way, a connection with fault
\DIFaddbegin \DIFadd{trace }\DIFaddend F1
...
accommodate diffuse stress within the ICH.   \DIFdelbegin \DIFdel{The median valley events
are sparse enough that it is just possible to interpret them as lying
on the northward continuations of the low angle faults identified in the middle section.  On the other hand, bathymetric structures }\DIFdelend \DIFaddbegin \DIFadd{Bathymetric structures
in and near the MV }\DIFaddend become
...
activity.  The ridge coincides with an \DIFaddbegin \DIFadd{apparently }\DIFaddend aseismic zone where
earthquakes, if they occur at all, are of much smaller magnitude
than
those in the MVSZ\DIFdelbegin \DIFdel{(during }\DIFdelend \DIFaddbegin \DIFadd{. During }\DIFaddend the experiment only events with a magnitude
less than 1 could have feasibly escaped detection\DIFdelbegin \DIFdel{). }\DIFdelend \DIFaddbegin \DIFadd{. Although 
...
nevertheless notable.  }\DIFaddend Elevated temperatures or the presence of fluids
...
\subsection{Synthesis with previous \DIFdelbegin \DIFdel{micro-earthquake }\DIFdelend \DIFaddbegin \DIFadd{microearthquake }\DIFaddend surveys}

\DIFdelbegin \DIFdel{The tectonic interpretation just put forward is broadly consistent
with \mbox{%DIFAUXCMD
\citeauthor{tucholke94}}%DIFAUXCMD
's }%DIFDELCMD < [%%%
\DIFdel{\mbox{%DIFAUXCMD
\citeyear{tucholke94}}%DIFAUXCMD
}%DIFDELCMD < ] %%%
\DIFdel{conceptual
...
faults (10--15~km, Figure~4 of \mbox{%DIFAUXCMD
\citet{ranero99}}%DIFAUXCMD
). At the other extreme
}%DIFDELCMD < {\it %%%
\DIFdel{Canales et al.}%DIFDELCMD < } [%%%
\DIFdel{``Seismic reflection imaging of a young
megamullion: Atlantis Massif (Mid-Atlantic Ridge, 30}%DIFDELCMD < \dg%%%
\DIFdel{, 10'N)'' submitted to }%DIFDELCMD < {\it %%%
\DIFdel{Earth Planet. Sci. Let.}%DIFDELCMD < }]
%DIFDELCMD < %%%
\DIFdel{have tentatively identified detachments with breakaways as little as
2~km apart in reflection data from the Atlantis core complex at 30}%DIFDELCMD < \dg %%%
\DIFdel{N.
}\DIFdelend %DIF > \remark{CUT THE FOLLOWING?:
...
presented by the segment \DIFaddbegin \DIFadd{south of the Oceanographer's Transform }\DIFaddend at 35\dg N \citep{barclay01} where earthquake
depths only reach 4~km \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{below the MV floor, a 
large moment $b$ value of 0.94 is found, }\DIFaddend and cross-axis relief is small.  
Based on
...
\DIFaddbegin \DIFadd{In spite of the fact that the segment north of the
Oceanographer's Transform has been classified as hot by
\mbox{%DIFAUXCMD
\citet{thibaud98}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{cessaro86} }%DIFAUXCMD
find a low $b$ value of 0.7 or
...
\DIFaddend %, whereas the segment at 29\dg N has been
...
slightly less than the depth indicated by \DIFdelbegin \DIFdel{micro-earthquakes}\DIFdelend \DIFaddbegin \DIFadd{microearthquakes}\DIFaddend .
...
saturates at ~8 km \DIFaddbegin \DIFadd{(below the MV floor)}\DIFaddend .
...
extension before the proposed ridge jump.  \DIFdelbegin \DIFdel{The active faults at 5}%DIFDELCMD < \dg %%%
\DIFdel{S\ are planar. }\DIFdelend Although it is possible and even likely that
over time \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{the faults at 5}\dg \DIFadd{S }\DIFaddend will exhume their footwalls, there is currently no need to accommodate  bending
...
an \DIFdelbegin \DIFdel{early micro-earthquake }\DIFdelend \DIFaddbegin \DIFadd{earlier microearthquake }\DIFaddend OBS survey near \DIFdelbegin \DIFdel{the Vema transform fault at
11}%DIFDELCMD < \dg %%%
\DIFdel{N }\DIFdelend \DIFaddbegin \DIFadd{major
transforms  }\DIFaddend also reported diffuse \DIFdelbegin \DIFdel{micro-earthquake }\DIFdelend \DIFaddbegin \DIFadd{microearthquake }\DIFaddend activity at the inside
corner \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{rowlett84}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citetext{Vema Transform, 11\dg N, \citealp{rowlett84};
Oceanographer's Transform, 35\dg N, \citealp{cessaro86}}}%DIFAUXCMD
}\DIFaddend , so the question is open.  
...
depth (8~km below the median valley floor) and \DIFaddbegin \DIFadd{moment }\DIFaddend $b$ values (0.8) in the MVSZ, as well as the large
...
accommodated along mantle-penetrating  \DIFdelbegin \DIFdel{low-angle normal faults, but in contrast to
...
inner fault}\DIFdelend \DIFaddbegin \DIFadd{normal faults}\DIFaddend .
\DIFaddbegin \DIFadd{Seismic activity  in the mantle and only moderately low
velocities beneath the Moho preclude a large degree of
serpentinisation there.
}\DIFaddend The presence of a well-defined volcanic
ridge and the absence of recorded earthquakes near the segment center
indicate \DIFdelbegin \DIFdel{it is }\DIFdelend \DIFaddbegin \DIFadd{that it might be }\DIFaddend hot and magmatically active.  This contrast between
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ignore{%In many cases, the error thus determined were far less
...
part of the picking
error was estimated }%DIFDELCMD < {\it %%%
\DIFdel{a posteriori}%DIFDELCMD < } %%%
\DIFdel{following the procedure of \mbox{%DIFAUXCMD
\citet{wilcock91}}%DIFAUXCMD
,
...
independent but result from the weights assigned }%DIFDELCMD < {\it %%%
\DIFdel{a priori}%DIFDELCMD < } %%%
\DIFdel{to the
...
hypocenters.  
%DIF < In many cases, the error thus determined were far less
%DIF < than the formal errors reported by the location
%DIF < routine \citep[program HYP, ][]{lienert95} because this code estimates
%DIF < picking errors for each event separately, which results in a
%DIF < significant over-estimation if there are only few picks available per event\citep{wilcock91}.
%DIF < In other cases the Monte-Carlo error was larger than the formal error
%DIF < because the formal error did not properly take into account the non-linearity
%DIF < of the location problem.  The 95\%  errors are mostly between ..  and
%DIF < .. km horizontally, and between ... and .. vertically (see
%DIF < Table~\ref{tbl:bulletin}).
...
All events west of 11}%DIFDELCMD < \dg %%%
...
%DIF < (it has to be remembered that it is rather unlikely that a large
%DIF < number of events are at either bound). 
...
station terms and a minimum 1D-model \mbox{%DIFAUXCMD
\citep[using
VELEST,][]{kissling94}}%DIFAUXCMD
...
perpendicular to the ridge }%DIFDELCMD < [{\it %%%
\DIFdel{Planert et al.}%DIFDELCMD < }%%%
\DIFdel{,
in preparation}%DIFDELCMD < ]%%%
...
%DIF < the density of the refraction profiles is not high enough to
%DIF < make such an enterprise feasible. 
%DIF < \remark{ I THINK IT IS 
%DIF < FEASIBLE AND WOULD BE VERY WORTHWHILE TO CONSTRUCT AND USE A 3D MODEL
%DIF < BASED ON A COMBINATION OF REFRACTION DATA, TOPOGRAPHY AND POSSIBLY GRAVITY
%DIF < BUT IT IS NOT A TRIVIAL TASK AND I AM NOT WELL POSITIONED TO CARRY IT
%DIF < OUT, BOTH TIME-WISE AND WITH RESPECT TO ACCESS TO  THE REFRACTION DATA, WHICH IS
%DIF < THE MAIN CONSTRAINT.  MAYBE LARS OR SOMEONE ELSE  FEELS INCLINED.  ONCE A 3D MODEL EXISTS, IT IS RELATIVELY STRAIGHTFORWARD
%DIF < TO RE-LOCATE THE EQS IN IT, AND I COULD EASILY CARRY THAT OUT.}  
...
%DIFDELCMD < \item[Modeling of wave propagation]
%DIFDELCMD < %%%
\DIFdel{A further systematic error arises from the assumptions made in
...
oceanic lithosphere can be  strongly anisotropic \mbox{%DIFAUXCMD
\citep{barclay98}}%DIFAUXCMD
.  However, the
...
%DIFDELCMD < %%%
\DIFdelend \begin{acknowledgments}
...
and picking.  \DIFaddbegin \DIFadd{Comments by Douglas Toomey and an anonymous reviewer
and associate editor helped to focus and clarify the paper.
}\DIFaddend Software packages contributing substantially to the
analysis, processing and presentation of this data set were GMT \citep{wessel91},
SeisAn \citep{havskov99}, and the
PASSCAL suite of programs\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend part of the trigger
...
\DIFdelbegin %DIFDELCMD < \end{article}
%DIFDELCMD < %%%
%DIF <  for galley place end article before bibliography!
%DIFDELCMD < 
...
\DIFaddendFL \caption{Focal mechanism solutions}
...
\end{tabular}
\end{flushleft}
\DIFdelbeginFL %DIFDELCMD < \tablecomment{5 6 0939: Solutions with strikes 340-10{\dg} are
%DIFDELCMD < compatible with the data. 340{\dg} was chosen as the strike of the
%DIFDELCMD < preferred solution because it is parallel to the structure in the
%DIFDELCMD < topography.  The dip is constrained to lie between 25\dg\ and 40\dg, the
%DIFDELCMD < rake can lie between -70 and -90\dg (with the lower bound only
%DIFDELCMD < obtained for strikes close to 0\dg). 
%DIFDELCMD < % Errors for other events give the approximate range of solutions
%DIFDELCMD < %compatible with the polarity measurements. 
%DIFDELCMD < Polarity errors for both events are close to nodal planes (see Fig~\protect\ref{fig:seismag-map}).} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \tablecomment{5 6 0939: Solutions with strikes 340-10{\dg} are
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table*}
\DIFaddendFL \caption{Comparison of OBS surveys in the North Atlantic \DIFdelbeginFL \DIFdelFL{with North Atlantic regional hydrophone array data}\DIFdelendFL }

\begin{flushleft}
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{clllcrcrc}
%DIFDELCMD < \tableline
%DIFDELCMD <                  %%%
\DIFdelendFL %DIF > \begin{tabular}{clllcrcrc}
%DIF > \tableline
%DIF >                  &           &           & Max   &                  &\multicolumn{2}{l}{Cumul.\#OBS}   & \multicolumn{2}{l}{Cumul.\#Hydro}  \\ 
%DIF >                  &           &           & Depth & $b$-value        &\multicolumn{2}{l}{$M_0>10^{19}$dyn-cm\tm{c}}   & \multicolumn{2}{l}{ASL$>$211 db\tm{d}} \\
%DIF > Area\tm{a}       & Reference & \#days   & (km)\tm{b} & ($\log_{10}M_0$) &\multicolumn{1}{l}{Total} & Per week&\multicolumn{1}{l}{Total} &Per week\\
%DIF > \tableline
%DIF > 22\dg 30'-22\dg 50' N &\citet{toomey88} & 10  & 8 & $0.8\pm0.1$ (MV floor) &         12 & 8.4 & 35 & 0.23 \\
%DIF >                       &                 &     & 5 & $0.5\pm0.1$ (Rift Mnts.)    &            &     &    &      \\
%DIF > 26\dg 00'-26\dg 13' N &\citet{kong92}   & 23  &   & $1.0\pm0.1$ (total)       &         93 &28.3 & 32 & 0.21 \\
%DIF >                       &                 &     & 7  &0.6-0.9$\pm$0.1 (segm.end)&            &     &    &      \\ 
%DIF >                       &                 &     & 6  &1.1-1.5$\pm$0.1 (segm.cnt.)&            &     &    &      \\
%DIF > 28\dg 52'-29\dg 05' N &\citet{wolfe95}& 41    & 5.5-7\tm{e}  & $0.82\pm0.05$   &  not known      & &  44 & 0.04 \\ 
%DIF > \multicolumn{2}{r}{(Segment end and ICH only)} &  &   &                     &            &     &    &      \\
%DIF > 34\dg 42'-35\dg 00' N &\citet{barclay01}& 43  & 4   &$0.94\pm0.05$            &          4 &0.65 &  3 & 0.02 \\
%DIF >                       & (Segment center only) &   &    &                     &            &     &    &      \\
\DIFaddbeginFL \begin{tabular}{clllcrc}
                 \DIFaddendFL &           &           & Max   &                  &\multicolumn{2}{l}{Cumul.\#OBS}   \DIFdelbeginFL %DIFDELCMD < & \multicolumn{2}{l}{Cumul.\#Hydro}  %%%
\DIFdelendFL \\ 
                 &           &           & Depth & $b$-value        &\multicolumn{2}{l}{$M_0>10^{19}$dyn-cm\tm{c}}  \DIFdelbeginFL %DIFDELCMD < & \multicolumn{2}{l}{ASL$>$211 db\tm{d}} %%%
\DIFdelendFL \\
Area\tm{a}       & Reference & \#days   & (km)\tm{b} & ($\log_{10}M_0$) &\multicolumn{1}{l}{Total} & Per week \DIFdelbeginFL %DIFDELCMD < &\multicolumn{1}{l}{Total} &%%%
\DIFdelFL{Per week}\DIFdelendFL \\
\tableline
22\dg 30'-22\dg 50' N &\citet{toomey88} & 10  & 8 & $0.8\pm0.1$ (MV floor) &         12 & 8.4  \DIFdelbeginFL %DIFDELCMD < & %%%
\DIFdelFL{35 }%DIFDELCMD < & %%%
\DIFdelFL{0.23 }\DIFdelendFL \\
                      &                 &     & 5 & $0.5\pm0.1$ (Rift Mnts.)    &            &     \DIFdelbeginFL %DIFDELCMD < &    &      %%%
\DIFdelendFL \\
26\dg 00'-26\dg 13' N &\citet{kong92}   & 23  &   & $1.0\pm0.1$ (total)       &         93 &28.3  \DIFdelbeginFL %DIFDELCMD < & %%%
\DIFdelFL{32 }%DIFDELCMD < & %%%
\DIFdelFL{0.21 }\DIFdelendFL \\
                      &                 &     & 7  &0.6-0.9$\pm$0.1 (segm.end)&            &       \DIFdelbeginFL %DIFDELCMD < &    &      %%%
\DIFdelendFL \\ 
                      &                 &     & 6  &1.1-1.5$\pm$0.1 (segm.cnt.)&            &    \DIFdelbeginFL %DIFDELCMD < &    &      %%%
\DIFdelendFL \\
28\dg 52'-29\dg 05' N &\citet{wolfe95}& 41    & 5.5-7\DIFdelbeginFL %DIFDELCMD < \tm{e}  %%%
\DIFdelendFL \DIFaddbeginFL \tm{d}  \DIFaddendFL & $0.82\pm0.05$   &  not known      &  \DIFdelbeginFL %DIFDELCMD < &  %%%
\DIFdelFL{44 }%DIFDELCMD < & %%%
\DIFdelFL{0.04 }\DIFdelendFL \\ 
\multicolumn{2}{r}{(Segment end and ICH only)} &  &   &                     &            &     \DIFdelbeginFL %DIFDELCMD < &    &      %%%
\DIFdelendFL \\
34\dg 42'-35\dg 00' N &\citet{barclay01}& 43  & 4   &$0.94\pm0.05$            &          4 &0.65  \DIFdelbeginFL %DIFDELCMD < &  %%%
\DIFdelFL{3 }%DIFDELCMD < & %%%
\DIFdelFL{0.02 }\DIFdelendFL \\
                      & (Segment center only) &   &    &                     &            &        \DIFaddbeginFL \\
\DIFaddFL{35}\dg \DIFaddFL{00'-35}\dg \DIFaddFL{15' N }\DIFaddendFL &\DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citet{cessaro86} }%DIFAUXCMD
}\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{12 }& \DIFaddFL{9 (14) }\tm{e} & \DIFaddFL{0.5-0.7}\tm{f} & \DIFaddFL{not known }& \DIFaddendFL \\
                      \DIFaddbeginFL & \DIFaddFL{(Segment end only) }&   &    &                     &            &        \\
\DIFaddendFL \tableline
\end{tabular}
\end{flushleft}
...
\DIFdelbeginFL %DIFDELCMD < \tablenotetext{d}{ASL is the acoustic source level as defined in
%DIFDELCMD < \citet{fox01} (the hydrophone catalogue is estimated to be complete
%DIFDELCMD <  to an ASL of 210 db).  According to their regression, an ASL of 211
%DIFDELCMD < db corresponds to an earthquake of magnitude 2.0.  Taken at face
%DIFDELCMD < value, this would imply an excess of OBS events relative to the events
%DIFDELCMD < recorded by the hydrophone array, however, the magnitude-ASL
%DIFDELCMD < regression presented by \citet{fox01} has large error bounds, and in
%DIFDELCMD < fact, an ASL of 211~db could correspond to earthquakes with magnitudes
%DIFDELCMD < as large as 3, if the upper error bound is used for the slope of the
%DIFDELCMD < ASL versus magnitude curve (9.9 instead of 7.8 db/mag).  This seems
%DIFDELCMD < reasonable given that the $b$ value of the frequency-ASL curve,
%DIFDELCMD < $\sim$12, translates into the average slow-spreading ridge frequency-magnitude $b$ value of
%DIFDELCMD < 1.2--1.3 \citep{rundquist02} only if the upper bound is assumed.  The hydrophone
%DIFDELCMD < event rate then should be multiplied by a factor of 15--20 for a
%DIFDELCMD < direct comparison with OBS rates. Both rates are then compatible at
%DIFDELCMD < least by order of magnitude, with the possible exception of \citet{kong92}. }
%DIFDELCMD < \tablenotetext{e}{Lower bound for earthquakes located with at least 5 OBS, upper bound for earthquakes located with 4 OBS.}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \tablenotetext{d}{Lower bound for earthquakes located with at least 5 OBS, upper bound for earthquakes located with 4 OBS.}
...
\DIFaddendFL \label{tbl:comparison}
\DIFdelbeginFL %DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \end{table*}
\DIFaddend 
...
\DIFaddendFL \includefig{glob-seis/glob-seis}{35pc}%.ps
...
faults.   The  \DIFdelbeginFL \DIFdelFL{yellow }\DIFdelendFL rectangle marks the location of the detailed map
in Figure~2.   \DIFdelbeginFL \DIFdelFL{Red circles }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Circles }\DIFaddendFL indicate earthquakes from 1963-2001 in the
...
intervals.  \DIFdelbeginFL \DIFdelFL{Red }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL lines indicate the position of refraction profiles
...
\begin{plate*}
%DIF > \includefig{seismag-map/seismag-map}{39pc}%.eps
%DIF > \begin{sidewaysfigure}
\DIFaddendFL \includefig{seismag-map/seismag-map}{39pc}%.eps
%DIF < \noindent\includegraphics[width=39pc]{seismag-map/seismag-map.ps}

%DIF > %%\includegraphics[angle=90,width=33pc]{seismag-map/seismag-map}%.eps
\DIFaddbeginFL 

\DIFaddendFL \caption{Distribution of earthquakes recorded with the ocean bottom
...
\DIFdelbeginFL %DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \end{plate*}
%DIF > \end{figure}
\DIFaddend 
...
\begin{figure}
\DIFdelbeginFL \DIFdelFL{\includefig{data-example/data-example}{20pc}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\includefig{data-example/data-example-largelabel}{20pc}}\DIFaddendFL %.eps
...
Analyst picks are \DIFdelbeginFL \DIFdelFL{shown in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{marked with phase (color: }\DIFaddendFL red \DIFaddbeginFL \DIFaddFL{vertical line) }\DIFaddendFL , and calculated travel times for the \DIFdelbeginFL \DIFdelFL{best solution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inferred location }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{shown in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{marked with prefix ``y'' (color: }\DIFaddendFL blue \DIFdelbeginFL \DIFdelFL{.  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{vertical line)   }\DIFaddendFL Picks with the number 4 next to them
...
\begin{figure}
\DIFdelbeginFL \DIFdelFL{\includefig{velmod/velmod}{20pc}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\includefig{velmod/velmod-bw}{20pc}}\DIFaddendFL %.ps
...
\begin{figure*}
\DIFaddendFL \large \sf \bf a\hspace{17pc}b
...
of the profiles in b \DIFdelbeginFL \DIFdelFL{an }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL c, and the dashed line indicates the size of
...
\DIFdelbeginFL \DIFdelFL{Figure}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Plate}\DIFaddendFL ~\protect\ref{fig:seismag-map} for further details) in the
southernmost cross-section. 
Position of fault traces  F1 and F2 are marked (see \DIFdelbeginFL \DIFdelFL{Figure}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Plate}\DIFaddendFL ~\protect\ref{fig:seismag-map}).
...
\DIFaddbegin \end{article}
\clearpage


%DIF >  for galley place end article before bibliography!
\vspace*{10cm}
\DIFadd{All following texts, tables and figures are to be made available online only
as part of the supplementary material.
}\clearpage
%DIF > %%\input{uncertainty.tex}
\DIFaddend % AGU docu says the following commands should be used but they don't work
%\setfigurenum{1}
%\settablenum{1}
\setcounter{figure}{0}
