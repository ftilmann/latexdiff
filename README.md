...
LATEXDIFF-UI (Author: Tan Jing Jie) is a browser-only web application (Demo: https://latexdiff.web.app/) that runs latexdiff entirely on the client via WebAssembly and WebPerl (through the wasm-latex-tools package). Users paste or drop old and new .tex files into a three-column editor, set common latexdiff options (--type, --subtype, --floattype, --math-markup, --encoding, --flatten, --allow-spaces, and others), and copy or download the generated diff.tex without uploading sources to a server. It is intended as a lightweight front-end for authors who want tracked-changes markup without installing Perl or latexdiff locally.
https://github.com/jingjie00/latexdiff-ui
